...
# - 将定制的滤镜应用于图像（2D卷积）
# - 注意这里用的是卷积，不是矩阵相乘。（卷积：图像和卷积核的每个像素相乘再相加，得到一个值代替原先的值，卷积核在图像上滑动完成卷积运算）
...
# 【方差代表样本的离散程度，越大离散程度越高】（https://blog.csdn.net/qinglongzhan/article/details/82348153）
# 【结论】高斯滤波器采用像素周围的邻域并找到其高斯加权平均值。滤波时会考虑附近的像素。它不考虑像素是否具有几乎相同的强度。它不考虑像素是否是边缘像素。
# 因此它也模糊了边缘，这个可以通过双边滤波解决
...
## 【双边滤波】属于边缘保护滤波方法， 去除噪声的同时保持边缘清晰锐利非常有效，与其他过滤器相比，该操作速度较慢，有美颜的效果
