...
#【结论】离中心点近的点频率越低，越亮表示幅值越大，可以这样考虑：低频表示变化比较低的点，如背景，低频在图像中比较多，导致中心位置一般幅值高所以很亮。
...
#img = cv.imread('../opencv-cpp/pic/wudian.png',0)
...
mask[(crow-rect_w):(crow+rect_w), (ccol-rect_w):(ccol+rect_w)] = 1#中心正方形区域为1，其余全为零
...
plt.subplot(143),plt.imshow(img_back, cmap = 'gray'),plt.title('img_back')
img_back = img_back/img_back.max()*255
cv.imwrite('./img_back.bmp', img_back)

#【频率图像滤波-高通滤波】--删除低频信息，保留边缘信息，采用高通滤波器【拉普拉斯】
# 方形框会出现“振铃”效应，应该采用高斯框，这里没再实验了 TODO
mask = np.ones((rows,cols,2),np.uint8)# 首先创建一个掩码，
mask[(crow-rect_w):(crow+rect_w), (ccol-rect_w):(ccol+rect_w)] = 0#中心正方形区域为0，其余全为1
...
plt.show()
img_back = img_back/img_back.max()*255
cv.imwrite('./img_back_low.bmp', img_back)
